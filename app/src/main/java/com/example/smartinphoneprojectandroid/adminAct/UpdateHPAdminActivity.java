...
import com.example.smartinphoneprojectandroid.RegisterActivity;
import com.example.smartinphoneprojectandroid.UpdateDeleteFragment;
import com.example.smartinphoneprojectandroid.databinding.ActivityUpdateHpadminBinding;
...
public class UpdateHPAdminActivity extends AppCompatActivity {
    ActivityUpdateHpadminBinding binding;
    String id;
    SessionManager sm;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityUpdateHpadminBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        sm = new SessionManager(UpdateHPAdminActivity.this);
        ...
        binding.removeHP.setOnClickListener(v->{
            new AlertDialog.Builder(UpdateHPAdminActivity.this)
                    .setTitle("Delete Handphone")
        ...
        binding.submitUpdateHP.setOnClickListener(v->{
            ...
            {
                Toast.makeText(UpdateHPAdminActivity.this, "Please fill the required field!", Toast.LENGTH_SHORT).show();
            } else {
            submitFunc(id,
...
//        Log.e("log", "deleteFunc: "+map.get(SessionManager.KEY_TOKEN) );
        GetData service = RetrofitClient.myRetrofitInstance().create(GetData.class);
        Call<UpdateHPResponse> call = service.deleteHP("Bearer "+map.get(SessionManager.KEY_TOKEN), id);
...
//                Log.e("log", "onResponse2: "+response.toString() );
                if (response.isSuccessful()) {
                    if (response.body().getStatus().toString().equals("200")) {
                        Toast.makeText(getApplicationContext(), "Delete Successfully", Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
                        Toast.makeText(getApplicationContext(), "Delete Failed 1", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(getApplicationContext(), "Delete Failed 2", Toast.LENGTH_SHORT).show();
                }
            }
